...
class ContentController extends Common {
	...
	public function storesearchAction(){
		...
		$query = $this->db->query('select * from fn_content_1_address where dizhi like "%'.$sf.'%" AND dizhi like "%'.$sq.'%"');
		
		$row=$query->result_array();
		foreach ($row as $key => $val) {
			$row[$key]['content']=strip_tags(html_entity_decode($val['content']));
		    $arr = $this->db->query('select * from fn_content_1 where id='.$val['id']);
		    $abs = $arr->result_array();
		    $row[$key]['ex']=$abs[0];
		}
