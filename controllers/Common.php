...
class Common extends CI_Controller {
    ...
    public function __construct() {
        ...
        if (defined('SITE_BDPING') && SITE_BDPING &&
                date('G') < 6 && $this->agent->is_robot() && !get_cookie('sb') && $this->db->query("SHOW TABLES LIKE '" . $this->db->dbprefix . "sb'")->row_array()) {
    ...
    protected function getFields($fields, $data = array()) {
        ...
        foreach ($fields['data'] as $t) {
            if ($this->namespace != 'admin' && !$t['isshow'])
                continue;
    ...
    protected function postEvent($data, $event, $action) {
        ...
        if (empty($Events) || empty($Events[$action]))
            return true;
    ...
    protected function watermark($file) {
        if (!$this->site['SITE_WATERMARK'] || $this->site['SITE_THUMB_TYPE'])
            return false;
    ...
    protected function userShow($data) {
        ...
        if ($this->session->is_set('user_id') && $this->session->get('user_id'))
            return true;
        if (get_cookie('member_id') && get_cookie('member_id') == $data['userid'] && $data['sysadd'] == 0)
            return true;
        return false;
    }
        ...
        if (isset($_POST['geetest_challenge']) && isset($_POST['geetest_validate'])) {
            ...
            if ($this->session->userdata('gtserver') == 1) {
                $result = $GtSdk->validate(
                        $_POST['geetest_challenge'], $_POST['geetest_validate'], $_POST['geetest_seccode']
                );
                if ($result == TRUE) {
                    return TURE;
    ...
    protected function getFormMember() {
        ...
        if ($data) {
            foreach ($data as $id => $t) {
                if (isset($t['setting']['auth']['siteuser']) && $t['setting']['auth']['siteuser'] && $t['setting']['auth']['site'] && in_array($this->siteid, $t['setting']['auth']['site']))
                    continue;
    ...
    protected function memberPost($data) {
        if (isset($data['siteuser']) && $data['siteuser'] && $data['site'] && in_array($this->siteid, $data['site']))
            return true;
        if (isset($data['memberpost']) && $data['memberpost']) {
            if (isset($data['modelpost']) && in_array($this->memberinfo['modelid'], $data['modelpost']))
                return true;
            if (isset($data['grouppost']) && in_array($this->memberinfo['groupid'], $data['grouppost']))
                return true;
        }
        return false;
    }
    ...
    private function _init() {
        ...
        } elseif (isset($site[$name]) && !$this->ismobile()) {
            $siteid = (int) $site[$name];
        } elseif (isset($mb[$name]) && $this->ismobile()) {
            $siteid = (int) $mb[$name];
            $is_mb = 1;
        }
        $siteid = max($siteid, 1);
