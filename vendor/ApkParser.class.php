...
class ApkParser{
    ...
    public function getDefaultCompany(){
        for ($id=0; true; $id++){
            $act = $this->getAttribute("manifest/application/meta-data[{$id}]", 'android:name');
            if (!$act) break;
            if ($act == 'DEFAULT_COMPANY') return $this->getMetadata($id);
        }
        return NULL;
    }
    ...
    public function getMetadata($idx=0){
        $idx = intval($idx);
        return $this->getAttribute("manifest/application/meta-data[{$idx}]", 'android:value');
    }
    ...
    private function getAttributeValue($a){
        ...
        switch ($type){
            ...
            case self::TYPE_ATTRIBUTE:
                return sprintf('?%sX', self::_getPackage($data), $data);
            case self::TYPE_REFERENCE:
                return sprintf('@%sX', self::_getPackage($data), $data);
            case self::TYPE_INT_HEX:
                return sprintf('0xX', $data);
        ...
        return sprintf('<0x%X, type 0xX>', $data, $type);
    }
