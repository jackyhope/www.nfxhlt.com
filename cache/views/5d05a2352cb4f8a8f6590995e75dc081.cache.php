...
<div class="subnav">
	...
	<div class="table-responsive mytable">
		...
		<tr>
			...
			<input <?php if (!admin_auth($userinfo['roleid'], 'form-edit')) { ?>disabled<?php } ?> type="submit" class="btn btn-success btn-sm" value="<?php echo lang('a-order'); ?>" name="submit_order" onClick="$('#list_form').val('order')" />&nbsp;
			<input <?php if (!admin_auth($userinfo['roleid'], 'form-del')) { ?>disabled<?php } ?> type="submit" class="btn btn-success btn-sm" value="<?php echo lang('a-del'); ?>" name="submit_del" onClick="$('#list_form').val('del');return confirm_del()" />&nbsp;
			<input <?php if (!admin_auth($userinfo['roleid'], 'form-edit')) { ?>disabled<?php } ?> type="submit" class="btn btn-success btn-sm" value="<?php echo lang('a-con-36'); ?>" name="submit_status_1" onClick="$('#list_form').val('status_1')" />&nbsp;
			<input <?php if (!admin_auth($userinfo['roleid'], 'form-edit')) { ?>disabled<?php } ?> type="submit" class="btn btn-success btn-sm" value="<?php echo lang('a-con-37'); ?>" name="submit_status_0" onClick="$('#list_form').val('status_0')" />&nbsp;
			<input <?php if (!admin_auth($userinfo['roleid'], 'form-edit')) { ?>disabled<?php } ?> type="submit" class="btn btn-success btn-sm" value="<?php echo lang('a-con-39'); ?>" name="submit_status_3" onClick="$('#list_form').val('status_3')" />&nbsp;
			<?php if ($join) {  echo lang('a-con-65'); ?>：
			<input type="text" class="input-text" size="10" name="toid" />
			<input <?php if (!admin_auth($userinfo['roleid'], 'form-edit')) { ?>disabled<?php } ?> type="submit" class="btn btn-success btn-sm" value="<?php echo lang('a-con-66'); ?>" name="submit_join" onClick="$('#list_form').val('join')" />&nbsp;
			<?php } ?>
			</td>
		</tr>
        <?php if ($diy_file) { ?>
